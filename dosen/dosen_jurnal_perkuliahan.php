	...
	<table class="table table-striped table-bordered table-hover" id="dataTables-example">
		...
		<tr  valign="top">
			...
			<td>
				...
				<form action="dosen_proses_simpan_materi-<?php echo str_replace("-","_yz_",$id_kelas); ?>-<?php echo str_replace("-","_yz_",$id_ptk); ?>-<?php echo $i; ?>-<?php echo $idJurnal; ?>.html" method="POST">
				
				<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="1" <?php if ($dataJurnal && $dataJurnal['kegiatan']=='1'){echo"checked";} ?> > Perkuliahan &nbsp </input>
				<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="2" <?php if ($dataJurnal && $dataJurnal['kegiatan']=='2'){echo"checked";} ?>> UTS &nbsp </input>
				<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="3" <?php if ($dataJurnal && $dataJurnal['kegiatan']=='3'){echo"checked";} ?>> UAS &nbsp </input>
				
				<textarea class="form-control" name="materi" required style="width:400px; height:40px" <?php echo $statusdisabled; ?>><?php echo ($dataJurnal ? $dataJurnal["materi"] : ""); ?></textarea>
			</td>
			
			<td>
				<input type="date" name="tanggal" required value="<?php echo ($dataJurnal ? $dataJurnal['tanggal'] : ''); ?>" class="form-control" style="width:160px;" <?php echo $statusdisabled; ?>></input>
			</td>
			<td>
				<select name="ruang" required class="form-control" style="width:160px;" <?php echo $statusdisabled; ?>>
					<option value="<?php echo ($dataJurnal ? $dataJurnal['ruang'] : ''); ?>"><?php echo ($dataJurnal ? $dataJurnal['ruang'] : ''); ?></option>
					<?php 
			...
			<td>
				<?php if ($dataJurnal && $dataJurnal['id_jurnal']!=''){ ?>
					<input type="submit" class="btn btn-warning" value="Simpan" name="lanjut" <?php echo $statusdisabled; ?> >
				<?php }else{?>
			...
			<td>
				<?php if ($dataJurnal && $dataJurnal['id_jurnal']!=''){ ?>
					<input type="submit" class="btn btn-danger" value="Hapus" name="hapus" <?php echo $statusdisabled; ?>>
				<?php } ?>
				</form>
			</td>
						<?php
