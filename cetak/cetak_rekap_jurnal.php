...
header("Content-Disposition: attachment; filename=jurnal-$_POST[bulan].xls");
...
while ($dataDosen = mysql_fetch_array($sqlDosen)) {
	...
	th_ak='" . $dataMataKuliah['th_ak'] . "'");
		...
		while ($dataKelasMataKuliah = mysql_fetch_array($sqlKelasMataKuliah)) {
			...
			if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'A') {
				$prodi = "MO";
			} else if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'B') {
				$prodi = "MI";
			} else if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'C') {
				$prodi = "KM";
			} else if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'D') {
				$prodi = "HT";
			} else if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'E') {
				$prodi = "Far";
			} else if (substr($dataKelasMataKuliah['progdi'], 0, 1) == 'F') {
				$prodi = "MIK";
			}
