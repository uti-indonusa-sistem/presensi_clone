<?php if ($_COOKIE['simpreskul_admin'] == '') {
		...
		<tr>
			...
			<td style="width:25%;">
				<font style="font-size:10pt;">
					<?php echo view_kelas($_GET['id_kelas'], $_GET['id_ptk'], "nama_dosen"); ?>
			</td>
			<td style="width:12%;">
				<font style="font-size:10pt;">
...
<table class="table table-striped table-bordered table-hover" id="dataTables-example">
	...
	<?php
	// Optimization: Query Class Info & Condition ONCE outside the loop
	$id_kls_clean = str_replace("_yz_", "-", $_GET['id_kelas']);
	$id_ptk_clean = str_replace("_yz_", "-", $_GET['id_ptk']);

	$sqlKelas = mysqli_query($connection, "SELECT*FROM viewKelasKuliah WHERE xid_kls='" . $id_kls_clean . "' 
            AND id_ptk='" . $id_ptk_clean . "'");
	$dataKelas = mysqli_fetch_array($sqlKelas);

	// Fix fatal error: Handle case where cek_gabungan is empty
	$condition_kelas = cek_gabungan($_GET['id_kelas']);
	if (empty($condition_kelas)) {
		$condition_kelas = "xid_kls='" . $id_kls_clean . "'";
	}

	// NEW Optimization: Fetch rooms once
	$rooms = array();
	$sqlRuang = mysqli_query($connection, "SELECT ruang FROM presensi_ruang ORDER BY id_ruang DESC");
	if ($sqlRuang)
		while ($r = mysqli_fetch_array($sqlRuang)) {
			$rooms[] = $r['ruang'];
		}
	...
	for ($i = 1; $i <= 16; $i++) { ?>
		...
		<tr valign="top">
			...
			<td>
				...
				<form
					action="admin_proses_simpan_materi-<?php echo str_replace("-", "_yz_", $_GET['id_kelas']); ?>-<?php echo str_replace("-", "_yz_", $_GET['id_ptk']); ?>-<?php echo $i; ?>-<?php echo $idJurnal; ?>.html"
					method="POST">

					<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="1" <?php if ($dataJurnal['kegiatan'] == '1') {
							echo "checked";
						} ?>> Perkuliahan &nbsp </input>
					<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="2" <?php if ($dataJurnal['kegiatan'] == '2') {
							echo "checked";
						} ?>> UTS &nbsp </input>
					<input type="radio" required name="kegiatan" <?php echo $statusdisabled; ?> value="3" <?php if ($dataJurnal['kegiatan'] == '3') {
							echo "checked";
						} ?>> UAS &nbsp </input>

					<textarea class="form-control" name="materi" class="form-control" required
						style="width:400px; height:40px" <?php echo $statusdisabled; ?>><?php echo $dataJurnal["materi"]; ?></textarea>
			...
			<td>
				<input type="date" name="tanggal" required value="<?php echo $dataJurnal['tanggal']; ?>"
					class="form-control" style="width:160px;" <?php echo $statusdisabled; ?>></input>
			</td>
			<td>
